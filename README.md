...
2. Đảm bảo Gradle sử dụng phiên bản **9.0** (hoặc 8.7+) cùng Android Gradle Plugin **8.7.2** và Hilt plugin **2.55**. Android Studio Narwhal 3 Feature Drop đã đóng gói các phiên bản này; nếu không dùng `gradlew`, hãy cấu hình trong `Settings > Build Tools > Gradle` để trỏ tới bản Gradle tương thích nhằm tránh lỗi `fileCollection(Spec)`.
   - Script sử dụng `androidComponents` để khởi tạo lại các cấu hình `*RuntimeClasspathCopy`, đánh dấu chúng chỉ dùng để phân giải phụ thuộc (`canBeConsumed = false`, `canBeResolved = true`, đồng thời ẩn khỏi UI) nhằm đáp ứng yêu cầu mới của Gradle 9 về phân tách cấu hình. Nếu dùng Gradle cũ hơn, thay đổi này vẫn tương thích ngược.
...
> Lưu ý: Repo không đính kèm Gradle Wrapper (.jar) hay model nhị phân nhằm tuân thủ yêu cầu “không tệp nhị phân”. Hãy sử dụng Gradle 9.0 (hoặc tối thiểu 8.7) cài sẵn trong Android Studio hoặc tự cài thủ công.
